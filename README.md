...
- [1. Mac Setup Guide](#1-mac-setup-guide)
  ...
  - [1.2. プログラミング環境](#12-プログラミング環境)
    ...
    - [1.2.4. Ricty Diminished](#124-ricty-diminished)
...
cask-fontsは「Ricty Diminished」のようなフォントを所蔵している。

cask-driversは「Logi Options +」のような入力端末のドライバを所蔵している。

### 1.2.4. Ricty Diminished

プログラミング用等幅フォントであり、似ている文字（Iとlと|、Oと0）を識別しやすく、全角空白が明示されるという特徴を持つ。ターミナルやVSCodeなどに設定すると良い。Homebrewでダウンロードする。ただし、Rictyの公式によればすでに前時代的なフォントであり、不具合も多く確認されていることから、新規にダウンロードすることは推奨されないとのこと。その場合はmacOS標準で、Xcodeのプレーンテキストフォントである「SF Mono  Medium」を使用すると良い。
